...
def search_for_updates():
    ...
    r = requests.get("https://github.com/9155/915/releases/latest")
    ...
    if THIS_VERSION not in result_string:
        ...
        print(f'''\n\n                               [{lr}!{w}] JOINER [{m}{THIS_VERSION}{w}] is OUTDATED''')
        soup = BeautifulSoup(requests.get("https://github.com/9155/915/releases").text, 'html.parser')
        ...
        if choice.lower() == 'y' or choice.lower() == 'yes':
            ...
            if os.path.basename(sys.argv[0]).endswith("exe"):
                with open("JOINER.zip", 'wb')as zipfile:
                    zipfile.write(requests.get(update_url).content)
                with ZipFile("JOINER.zip", 'r') as filezip:
                    filezip.extractall()
                os.remove("JOINER.zip")
                cwd = os.getcwd()+'\\JOINER\\'
                shutil.copyfile(cwd+'Changelog.md', 'Changelog.md')
                try:
                    shutil.copyfile(cwd+os.path.basename(sys.argv[0]), '915-Nuker.exe')
                except Exception:
                    pass
                shutil.copyfile(cwd+'README.md', 'README.md')                   
                shutil.rmtree('JOINER')
                input(f"                               [{g}#{w}] Update Successfully Finished!", end="")
                os.startfile("JOINER.exe")
                os._exit(0)

            else:
                new_version_source = requests.get("https://github.com/9155/915/archive/refs/heads/master.zip")
                with open("JOINER-main.zip", 'wb')as zipfile:
                    zipfile.write(new_version_source.content)
                with ZipFile("JOINER-main.zip", 'r') as filezip:
                    filezip.extractall()
                os.remove("JOINER-main.zip")
                cwd = os.getcwd()+'\\JOINER-main'
